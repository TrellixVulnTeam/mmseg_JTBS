...
CVC_CLINIC_DB_LEN = 612 * 2
TRAINING_LEN = 512 * 2


def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert CVC-ClinicDB dataset to mmsegmentation format')
    parser.add_argument('dataset_path', help='path of CVC-ClinicDB.zip')
...
def main():
    ...
    if args.out_dir is None:
        out_dir = osp.join('data', 'CVC-ClinicDB')
    else:
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        print('Extracting CVC-ClinicDB.zip...')
        zip_file = zipfile.ZipFile(dataset_path)
        zip_file.extractall(tmp_dir)

        print('Generating training dataset...')
        assert len(os.listdir(tmp_dir)) == CVC_CLINIC_DB_LEN, \
            'len(os.listdir(tmp_dir)) != {}'.format(CVC_CLINIC_DB_LEN)
